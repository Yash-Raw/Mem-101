"""Counting model calls, and finding the read path makes none.

The thesis this course opened with -- RAG is a read path, a memory layer is
dominated by the write path -- is measurable rather than rhetorical:

    write path (full ingest, 24 turns)   llm 48   embed 38
    read path  (one question)            llm  0   embed  2

**Zero.** The read path is scope filtering, slot matching, cached vectors and
arithmetic; every model call in the system happens while nobody is waiting for
an answer. That is the exact inverse of a retrieval stack, where the read path
is the expensive part and indexing is amortised.

Per turn the write path costs 2.0 model calls and 1.6 embeddings, and both
scale with the conversation rather than with the store -- which is the number
that matters when someone asks what a year of use costs.
"""
