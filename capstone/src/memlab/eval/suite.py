"""One harness over every profile, so a claim about a level is checkable.

`component-metrics` scores four stages against the key. `exam` scores one
question. Neither answers the question this course has been implicitly making
for three levels: **does the later profile actually beat the earlier one?**

A suite runs the same battery across `@I8`, `@A1` and later, and reports a
row per profile. Three things fall out that a single score cannot show:

    a metric that is flat across every profile is not measuring the work
    a metric that was already 1.000 at Level 2 cannot justify Level 3
    a regression is visible as a column that goes down

The last one is the point. Every claim in this course took the form "this
module improves X", and until now the evidence was a lesson quoting two
numbers measured by hand at different times.
"""
...
from ..eval.exam import exam_answer, exam_from_context
from ..pipeline import at
