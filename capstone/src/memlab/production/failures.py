"""Seven production failure classes, and where each one was already met.

Beginner catalogued seven failures of a naive system. This is the other seven
-- the ones that appear after it works, each recognisable by a symptom rather
than by a stage. Every one has been measured somewhere in this course, which
is what makes the field guide checkable rather than a list of worries.

The organising claim: **a production failure is identified by what the user
sees, not by which component is wrong.** The same symptom can come from three
stages, and the guide's job is to say which measurement distinguishes them.
"""
