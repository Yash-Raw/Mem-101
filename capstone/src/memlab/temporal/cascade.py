"""Validity that propagates through what was derived from it.

Two graphs run through this store and only one of them is the entity graph.

`graph-stores` measured the entity graph and found one node and no edges --
this corpus does not have the shape a knowledge graph is for, and A1's parser
did not change that. The *derivation* graph is a different structure: every
memory that was summarised, merged or corroborated carries `derived_from`, and
those edges are where validity has to travel.

They are also where it silently does not. Retiring a belief sets `invalid_at`
on that record and touches nothing that was built on top of it.
"""
