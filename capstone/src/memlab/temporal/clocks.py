...
The record has carried two timestamps since Beginner -- `happened_at` for when
a fact was true, `recorded_at` for when the system learned it. Having the
fields is not the same as populating them. This module measures the difference,
and names the four instants a bi-temporal record actually needs.

    event time   valid_from .. valid_to     when the fact was true in the world
    belief time  recorded_at .. invalid_at  when the system held it to be true

`happened_at` predates both and means "when this was asserted". It is kept
because a dozen Level 1 and 2 figures are measured against it; `valid_from`
falls back to it, so a record written before A1 still answers as-of queries --
just with whatever precision the assertion happened to carry.
"""
