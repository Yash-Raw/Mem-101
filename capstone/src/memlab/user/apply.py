"""Applying the model without saying more than was asked.

`from-facts-to-a-user-model` assembled six attributes. Injecting all six into
every context is the obvious use and the wrong one:
...
Every one of those volunteered sets contains `residence` -- her home address,
offered in reply to a question about lunch.

But "only what was asked" is also wrong, and the counterexample is in the same
model. `response_style` is *"prefers shorter answers"*, nobody will ever ask
about it, and applying it is the entire point of having remembered it.

So attributes divide by **how they are used**, not by how sensitive they are:

    ANSWER       state it when the question asks    employer, diet, residence
    INSTRUCTION  obey it always, state it never     response_style

An instruction that gets stated is a system leaking its own configuration. A
fact that gets volunteered is a system telling someone's address to whoever is
...
from ..retrieve.query import formulate, slots_for
from ..types import Scope
from .model import Attribute, UserModel
