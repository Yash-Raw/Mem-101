"""Lab: trace one turn through every box.
...
def trace(turn: dict, store: JsonlStore, scope: Scope, query: str, k: int = 5) -> Trace:
    """TODO: push `turn` through all six boxes, recording what each produced.

    Call t.add(box, produced, note) once per stage, in order:
      capture  -> [turn]
      extract  -> extract(turn, scope)
      resolve  -> resolve(memories, store.all())
      store    -> store.add(kept)
      retrieve -> EmbeddingRetriever().search(query, store.all(), scope, ...)
      assemble -> assemble(hits[:k])

    The notes are the point. Make each one say what the box did AND what it
    declined to do.
    """
    raise NotImplementedError("implement trace")


def _reference(turn: dict, store: JsonlStore, scope: Scope, query: str, k: int = 5) -> Trace:
    t = Trace()
...
def main() -> None:
    ...
    for n in (8, 12):
        store = JsonlStore(f"/tmp/memlab-anatomy-{n}.jsonl")
        store.clear()
        others = populate_except(store, scope, skip_session=n)
        print(f"\n(store pre-loaded with {others} memories from every other session)")
        for turn in [t for t in session(n) if t["role"] == "user"][:1]:
            print(f"\n=== session {n}: {turn['text'][:60]}\n")
            for stage in trace(turn, store, scope, QUESTION).stages:
                print(f"  {stage.box:<9} {stage.note}")
